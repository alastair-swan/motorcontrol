...
export default function Motor (){
        const [tabState, setTabState] = useState<{motor: number, motor0tab: number, motor1tab: number}>({
            motor: 0,
            motor0tab: 0,
            motor1tab: 0
        });
    ...
    const handleChange = (event: React.SyntheticEvent, newValue: number) => {
        setTabState({...tabState, motor: newValue});
    };
    ...
    const [motor0State, setMotor0State] = useState<MotorParams>({
        CP_LOW: false,
        TSD: false,
        ISD: false,
        OV_SPD: false,
        ...
        SLAVE_ADRS: RegisterList.SLAVE_ADRS.default[0] as number,
        SPD: RegisterList.SPD.default,
        hz_cnt: 0
    })
    const [motor1State, setMotor1State] = useState<MotorParams>({
        CP_LOW: false,
        TSD: false,
        ISD: false,
        OV_SPD: false,
        ...
        SINK: RegisterList.SINK.default,
        COMP_HYS: RegisterList.COMP_HYS.default,
        SLAVE_ADRS: RegisterList.SLAVE_ADRS.default[1] as number,
        SPD: RegisterList.SPD.default,
        hz_cnt: 0
    })
    ...
    return (
        <Box padding={2} height={'100vh'} overflow={'auto'}>
            <Tabs value={tabState.motor} onChange={handleChange} centered>
                    <Tab label={Label("Motor 0")} {...a11yProps(0)} />
                    <Tab label={Label("Motor 1")} {...a11yProps(1)} />
            </Tabs>
            <TabPanel value={tabState.motor} index={0}>
                <Grid2 container columns={1} spacing={1} width={1500}>
                    <Grid2 size={1} gridColumn={0}><MotorState motorNumber={0} state={motor0State} setState={(motorState: MotorParams) => {setMotor0State({...motorState})}}/></Grid2>
                    <Grid2 size={1} gridColumn={0}>
                        <MotorControlSettings 
                            ...
                            state={motor0State} 
                            setState={(motorState: MotorParams) => {setMotor0State({...motorState})}}
                            tab={tabState.motor0tab}
            ...
            <TabPanel value={tabState.motor} index={1}>
            <Grid2 container columns={1} spacing={1} width={1500}>
                    <Grid2 size={1} gridColumn={0}><MotorState motorNumber={1} state={motor1State} setState={(motorState: MotorParams) => {setMotor1State({...motorState})}}/></Grid2>
                    <Grid2 size={1} gridColumn={0}>
                        <MotorControlSettings 
                            motorNumber={1} 
                            sectionbgColor={sectionbgColor} 
                            itembgHoverColor={itembgHoverColor} 
                            itembgColor={itembgColor} state={motor1State} 
                            setState={(motorState: MotorParams) => {setMotor1State({...motorState})}}
                            tab={tabState.motor1tab}
                            setTab={(newTab: number) => {
                                setTabState({ ...tabState, motor1tab: newTab })}
                            }/>
                    </Grid2>
                </Grid2>
            </TabPanel>
...
export function MotorControlSettings({ motorNumber, sectionbgColor, itembgHoverColor, itembgColor, state, setState, tab, setTab}: 
    {motorNumber: number, sectionbgColor: string, itembgHoverColor: string, itembgColor: string, state: MotorParams, setState: (motorState: MotorParams) => void , tab: number, setTab: (newTab: number) => void}) {
